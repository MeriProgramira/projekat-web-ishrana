...
@section('content_user')
    ...
    <div class="testbox">
      <form action="{{ route('update-producer', $producer) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="banner">
          <h1>"Zdrava ishrana" - izmjeni proizvodjaca</h1>
        </div>

        <div class="item">
          <label for="title">Ime proizvodjaca<span>*</span></label>
          <input id="title" type="text" name="title" value="{{ $producer->title }}" />
          @error('title')
        ...
        <div class="item">
            ...
            <input id="location" type="text" name="location" value="{{ $producer->location }}" />
            @error('location')
              <div class="alert text-danger py-1">{{ $message }}</div>
            @enderror
          </div>
          <div class="item">
            ...
            <input id="products" type="text" name="products" value="{{ $producer->products }}" />
            @error('products')
              <div class="alert text-danger py-1">{{ $message }}</div>
            @enderror
          </div>
        <div class="item">
          ...
          <textarea name="about" id="content" cols="30" rows="10">{{ $producer->about }}</textarea>
