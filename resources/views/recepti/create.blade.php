...
@section('content_user')
    ...
    <div class="testbox">
      <form action="{{ route('create-recept') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="banner">
          <h1>"Zdrava ishrana" - unesi recept</h1>
        </div>
        ...
        <div class="item">
            ...
            <input id="ingredients" type="text" name="ingredients" value="{{ old('ingredients') }}" />
            @error('ingredients')
              <div class="alert text-danger py-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="btn-block">
          <button type="submit" >Sacuvaj recept</button>
        </div>
      </form>
    </div>
